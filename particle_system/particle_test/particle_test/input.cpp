...
void keyboardFunc (unsigned char key, int x, int y)
{
	switch (key)
	{
		...
		case 'c':
			particle_system->up_external_force = 0.0;
			particle_system->down_external_force = 0.0;
			particle_system->left_external_force = 0.0;
			particle_system->right_external_force = 0.0;
			break;
...
void SpecialKeys(int key, int x, int y)
{
	if (key == GLUT_KEY_UP)
	{
		particle_system->up_external_force += 0.1;
	}

	if (key == GLUT_KEY_DOWN)
	{	
		particle_system->down_external_force += 0.1;
	}

	if (key == GLUT_KEY_LEFT)
	{	
		particle_system->left_external_force += 0.1;
	}

	if (key == GLUT_KEY_RIGHT)
	{	
		particle_system->right_external_force += 0.1;
	}
}
